...
typedef struct tain_state_s{
    int length;
    int state;
    int pre_length;// 如果是注册和登录，用于记录用户名长度，如果是命令，用于记>
    char buf[1000];
} train_state_t;
...
typedef struct task_s{
    ...
    char data[1000];
    clientQueue_t *pclientQueue;//子线程要操作已连接队列，里面根据我的套接字可以得到user_id
                                //通过user_id ,一切我都有了
    int epfd;//这里是因为我token验证，如果通过了的话，我就要把当前fd移除
             //因为我后面这条tcp连接只用来传输文件
...
int recvFile(int sockFd, int user_id);
